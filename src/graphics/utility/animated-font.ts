...
class AnimatedFont {
    ...
    getGlyphInfo(glyph: string) {
        if(this._glyphs[glyph]) return this._glyphs[glyph];
        return this._glyphs['?'];
    }
    ...
    renderText(text: string): AnimatedString {
        ...
        for(const glyph of text) {
            ...
            for(let frame = 0; frame < this._frames; frame++) {
                ctxt.drawImage(this._image, info.x + frame * this._frameWidth, info.y, info.w, info.h,
                    caretX + frame * totalWidth, 0, info.w, info.h);
            }
            caretX += info.w;
        }
